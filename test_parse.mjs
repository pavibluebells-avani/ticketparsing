...
const result = parseMessage(text, "Test Group", "test@g.us", "TEST123", Date.now()/1000, "sender", "Tester");

console.log("Lottery:", result.lottery);
console.log("Timeslot:", result.timeslot);
console.log("Entries:", result.entries.length);
for (const e of result.entries) {
  console.log(`  number=${e.number} betType=${e.betType} qty=${e.qty} rate=${e.rate} category=${e.category}`);
}
